# FRLG OCR 170a 日文增强版 · r4

基于 EasyCon `1aed001c0e2d3a32d211c39bec26546741626bd6`，程序版本 `1.7.0-alpha.frlg-jpn.4`。本包支持日文名称、性格、摘要等级、六项能力值、野生等级，保留已实测可用的日英 TID 管线。
...
- Paddle 至少两个阈值给出高置信完整一致名称时接受。低置信、模糊匹配或候选冲突时，调用 `jpn.traineddata` Tesseract 取得第二意见；仍不一致则返回空字符串。性格匹配 25 项标准词，单独处理固定后缀；也可由 Paddle ≥90% 和 Tesseract ≥70% 的完整一致结果、加另一条 ≥85% 且距离至多 1 的 Paddle 结果共同确认，所有已接受候选须一致。
