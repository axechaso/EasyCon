# FRLG OCR 170a 日文增强版 · r8

基于 EasyCon `1aed001c0e2d3a32d211c39bec26546741626bd6`，程序版本 `1.7.0-alpha.frlg-jpn.8`。本包支持日文名称、性格、摘要等级、六项能力值、野生等级，保留已实测可用的日英 TID 管线。
...
- 模板数字须通过字模误差、候选差距、布局、数值范围检查；至少两组阈值完整一致且无冲突才接受。只有日版野生等级会初始化文字模型，其他数字场景不会。
