# FRLG OCR 170a 日文增强版 · r7

基于 EasyCon `1aed001c0e2d3a32d211c39bec26546741626bd6`，程序版本 `1.7.0-alpha.frlg-jpn.7`。本包支持日文名称、性格、摘要等级、六项能力值、野生等级，保留已实测可用的日英 TID 管线。

r7 根据日版实机 `ヒトデマン Lv19` 原始采集测试，把野生名称默认区域收净为 `290,127,360,73`：保留完整五字符名称，同时避开右侧紧邻的 `Lv` 首笔和状态框边缘。野生等级继续使用 `765,127,70,77`。脚本区域与程序默认区域保持一致。
...
```text
$名称 = OCR(290, 127, 360, 73, "FRLG_JPN_NAME:ミニリュウ|ハクリュー")
PRINT $名称
```
...
.\FrlgOcrReplay.exe .\capture.png FRLG_JPN_NAME --roi 290,127,360,73 --output .\replay-manual
```
